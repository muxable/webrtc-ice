...
func (a *Agent) clearSelectedPairs() {
	...
	a.selectedPairsLock.Lock()
	defer a.selectedPairsLock.Unlock()
	a.selectedPairs = []*CandidatePair{}
}

func (a *Agent) addSelectedPair(p *CandidatePair) {
	p.nominated = true
	a.selectedPairsLock.Lock()
	// verify that this candidate doesn't already appear.
	for _, q := range a.selectedPairs {
		if p.equal(q) {
			a.log.Tracef("received duplicate candidate pair: %s %s", p, q)
			a.selectedPairsLock.Unlock()
			return
		}
	}
	a.selectedPairs = append(a.selectedPairs, p)
	a.selectedPairsLock.Unlock()
